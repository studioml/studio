...
class SQSQueue(object):
    ...
    def dequeue(self, acknowledge=True):
        ...
        response = self._client.receive_message(
                    QueueUrl=self._queue_url)
        while 'Messages' not in response.keys() and counter < self._receive_timeout:
            self.logger.debug(('No messages received, sleeping and retrying ' +
                    '({} attempts left)...')
                    .format((self._receive_timeout - counter) // self._retry_time))
